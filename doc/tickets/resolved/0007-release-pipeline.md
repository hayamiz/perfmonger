...
確定スコープ C〜G および finding #1 のコード変更を機械的に適用し、
プロジェクト規定の検証を実施した。

### 変更内容

追加ファイル:
- `.goreleaser.yaml` — goreleaser v2 設定。`builds:` は `perfmonger` 単体
  （`dir: core` / `main: ./cmd/perfmonger` / `CGO_ENABLED=0` /
  linux amd64+arm64 / `ldflags: -s -w -X main.VERSION={{.Version}}`）。
  `archives:` は tar.gz で `COPYING`・`README.md`・`NEWS.md` を同梱。
  `checksum:` SHA256。`source: enabled: true`。`changelog: disable: true`。
- `.github/workflows/release.yml` — `v*.*.*` タグ push で発火。checkout
  (fetch-depth: 0) → setup-go 1.24 → setup-uv → ゲート
  `make build && make vet && make test` → NEWS.md 先頭節を awk で抽出 →
  `goreleaser/goreleaser-action@v6`（`~> v2`）で
  `release --clean --release-notes=/tmp/relnotes.md`。
  `permissions: contents: write` + `GITHUB_TOKEN`。
- `.github/workflows/ci.yml` — push(master/main) と pull_request で
  `make build && make vet && make test`。
- `RELEASING.md` — メンテナ向けリリース手順書（英語、README に合わせる）。
- `core/cmd/perfmonger/main_test.go` — root command の Version 配線を検証する
  ユニットテスト。

変更ファイル:
- `core/cmd/perfmonger/main.go` — `const VERSION = "0.14.4"` を
  `var VERSION = "dev"` に変更（finding #1）。`-ldflags -X` 注入の前提を満たす。
- `NEWS` → `NEWS.md` に `git mv` し、先頭エントリを 1.0.0 用に書き換え（D1）。
- `README.md` — Installation の "coming soon" プレースホルダを実手順
  （Releases リンク + `curl -L … | tar xz` 例）に置換（task F）。

注: 不要になった legacy バイナリ（`perfmonger-core` ディスパッチャ＋互換
シンボリックリンク）と Makefile 生成ルールの削除は本チケットのスコープ外で、
**#0011 に分離**したまま。

### テスト

- `core/cmd/perfmonger/main_test.go` を追加（`TestRootCommandVersionWired`）。
  `VERSION` が非空で root command の `Version` に配線されていることを検証。
  テストコード自体はコンパイル可能だが、同パッケージの既存
  `summary_test.go:131`（`validatePagerOption` 未定義）が **本変更前から**
  ビルド不能であり、`core/cmd/perfmonger` パッケージのテストバイナリが
  ビルドできない（unmodified tree でも再現）。これはプロジェクト規定の検証
  コマンド（`core/internal/perfmonger` のみ対象）の範囲外。VERSION 変更の
  動作はビルド済みバイナリで手動検証した（下記）。

### 検証結果

- `cd core/internal/perfmonger && go test -v -cover` … **PASS**（coverage 54.2%）
- `cd core/internal/perfmonger && go vet …` … **PASS**
- `make build` … **PASS**。`./lib/exec/perfmonger_linux_amd64 --version` →
  `PerfMonger version dev`（var 化が機能）。
- `uv sync && uv run pytest -v` … 36 passed, 5 failed。失敗 5 件は
  すべて `tests/test_background.py`（session 検出がサンドボックス環境で
  `unknown-session` になる）で、**本変更前のコードでも同一に再現する既存
  失敗**であり本チケットとは無関係。
- `goreleaser check` … **PASS**（1 config validated）。
- `goreleaser release --snapshot --clean` … **PASS**。amd64/arm64 両バイナリ、
  tar.gz アーカイブ（中身 = `perfmonger`・`COPYING`・`README.md`・`NEWS.md`）、
  ソース tar.gz、`checksums.txt`（SHA256）を生成。展開した binary の
  `--version` に snapshot バージョンが注入されることを確認（ldflags 注入の実証）。
- 3 つの YAML（ci.yml / release.yml / .goreleaser.yaml）は yaml.safe_load で
  well-formed を確認。

C〜G と finding #1 をすべて適用し、検証は（無関係な既存失敗を除き）すべて green。
